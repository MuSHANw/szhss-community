...
1. 第一阶段：核心功能补全（基础体验闭环）
  ...
  - [√] 增加入站答题机制（LV0 未认证用户通过答题后升级 LV1，解锁全部互动功能）。  
...
3. 第三阶段：内容与运营工具（社区活跃度提升）
  - [√] 增加社区任务/金币系统（鹏城币：连续登录、首次发帖、获赞达人等每日任务和成就任务）。
  - [√] 入站考试题库重构（从数据库迁移到 JS 文件，分两级考试，每日3次限制）。
  - [√] **圈子系统深度优化**（圈主管理面板、圈子公告、圈子活动、精华帖子筛选、成员管理、分享按钮）。
  - [ ] **自习室系统深度优化**（学习目标、自定义番茄钟、白噪音、加油互动、自习排行榜、完成庆祝动画）。
  - [ ] 实现帖子置顶、加精功能（管理员/圈主专用，配合圈子优化）。
  - [ ] 增加数据统计仪表盘（用户可查看个人发帖、互动趋势图表）。
  - [ ] 增加社区公告系统（管理员发布全站公告，首页顶部展示）。
  - [√] 增加举报与反馈处理状态的通知闭环（用户可追踪处理进度）。
  - [ ] 实现内容审核机制（敏感词过滤，自动拦截含敏感词的帖子/回复）。
4. 第四阶段：技术优化与部署（稳定与安全）
  - [√] 部署到公网（购买新加坡轻量云服务器，配置固定域名 szhss-community.top）。
  - [ ] 完成公安联网备案，网站底部悬挂备案号。
  - [ ] 增加移动端 PWA 支持（manifest.json + Service Worker，支持离线浏览和添加到桌面）。
  - [√] 实现全站 HTTPS 加密访问（Certbot 自动续期 Let's Encrypt 证书）。
  - [ ] 实现接口限流与防刷机制（express-rate-limit，防止恶意请求和爬虫）。
  - [ ] 实现数据定期备份机制（pg_dump + cron 定时任务，数据库每日自动备份）。
5. 第五阶段：生态扩展与未来展望（长期愿景）
  - [ ] 调研并制作微信小程序版本（降低用户使用门槛）。
  - [🔃] 开发手机 App（MING 正在使用 uni-app 进行移动端开发）。
